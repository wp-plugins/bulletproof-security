...
// and display string dump - also checks for single character "U" in string position 2 to validate wp-config.php status
function root_htaccess_status() {
	...
	_e('<font color="green"><strong>The .htaccess file that is activated in your root folder is:</strong></font><br>');
		var_dump($section);
		$check_string = strpos($section, "U");
		if ($check_string == "2") { // if you modify BPS .htaccess files this str pos must match for valid status checks
	...
	_e('<font color="red"><br><br>A BPS .htaccess file was NOT found in your root folder</font><br><br>');
	_e('<font color="red">wp-config.php is NOT .htaccess protected by BPS</font><br><br>');
	}
	}
	}
...
// Show WordPress DB Errors = false - Show DB errors should already be set to false in /includes/wp-db.php
// Extra insurance - this function will be expanded in the future to turn DB errors on and off from the Dashboard
