...
<h3><?php _e('Current Backed Up .htaccess Files '); ?><a href="javascript:void(0);" target="_blank" onmouseover="Tip('YOU ONLY NEED TO BACK UP YOUR EXISTING ORIGINAL HTACCESS FILES ONCE. Back up your existing htaccess files FIRST in case you have a problem when enabling any BulletProof Security modes. Once you have backed up your orginal existing htaccess files you will see them listed here. IF YOU BACKUP THE BULLETPROOF HTACCESS FILES AFTER YOU HAVE BACKED UP YOUR ORIGINAL HTACCESS FILES THEN YOUR ORIGINAL HTACCESS FILES WILL BE OVERWRITTEN. There is no need to EVER back up the BulletProof Security htaccess files because they are copies of master htaccess files stored in your /plugins/bulletproof-security/htaccess folder.<br><br>To permanently remove the red error message from displaying below if you did not have pre-existing .htaccess files for your root and or /wp-admin folders then just enable Default Mode for whichever folder did not contain an original existing .htaccess file and perform a One Time Backup. CAUTION: If you have backed up orginal .htaccess files in the /wp-content/plugins/bulletproof-security/backup folder they will be overwritten. Download any backed up files you find in the /backup folder first, then upload them back to the /backup folder after you have removed the red error message. Future releases of BulletProof will NOT require this additional step.', WIDTH, 400, PADDING, 8, ABOVE, true, FADEIN, 400, FADEOUT, 300)" onmouseout="UnTip()">Read Me!</a></h3>
